...
class SubjectDao:
    ...
    def insert(self, code: str, fullname: str, semester: int) -> int:
        ...
        if len(code) != 3 or len(fullname) == 0:
            ...
            try:
                session.add(Subject(code=code.upper(), fullname=fullname, semester=abs(semester)))
                session.commit()
                session.expunge_all()
                return 0
            except Exception as e:
    ...
    def find_by_code(self, filter: list[str]) -> list[Subject]:
        ...
        if not filter:
            ...
            try:
                ...
                q = session.query(Subject).filter(Subject.code.in_(filter)).all()
            except Exception as e:
                session.rollback()
                q = []
                print(f"Exception caught on SubjectDao: {e}")
    ...
    def find_one_by_code(self, code: str) -> Union[Subject, None]:
        ...
        if len(code) != 3:
            ...
            code = code.upper()
            q = session.query(Subject).filter(Subject.code == code).first()
            return q
    ...
    def find_by_semester(self, semester: int) -> list[Subject]:
        ...
        if semester > 8 or semester < 0:
            ...
            q = session.query(Subject).filter(Subject.semester == semester).all()
            return q
